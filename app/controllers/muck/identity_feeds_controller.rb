    ...
    respond_to do |format|
      ...
      format.json do
        render :json => { :success => true,
                          ...
                          :message => t("muck.services.identity_feed_removed") }
      end
    end
  end
  ...
  protected
    ...
    def has_identity_feed_permission?
      if !@identity_feed.can_edit?(current_user)
        message = I18n.t('muck.services.cant_modify_identity_feed')
        respond_to do |format|
          format.html do
    ...
    def add_feeds_to_parent(parent, feeds)
      ...
      feeds.each do |feed|
        begin
          ...
          success = true
          if params[:username]
            messages << I18n.t('muck.services.successfully_added_username_feed', :service => feed.title || @service.name)
          else
            messages << I18n.t('muck.services.successfully_added_uri_feed', :service => @service.name, :uri => feed.title || params[:uri])
          end
        rescue ActiveRecord::RecordInvalid => ex
          if ex.to_s == 'Validation failed: Feed has already been taken'
            if params[:username]
              messages << I18n.t('muck.services.already_added_username_feed', :service => feed.title || @service.name, :username => params[:username])
            else
              messages << I18n.t('muck.services.already_added_uri_feed', :service => @service.name, :uri => feed.title || params[:uri])
            end
          end
        end
      end
